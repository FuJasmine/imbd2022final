...
print('Reading sg...')
sg = [pd.read_csv('train1/' + str(i) + '_sg.csv') for i in range(1, 47)]
print('Reading spike...')
spike = [pd.read_csv('train1/' + str(i) + '_spike.csv') for i in range(1, 47)]
print('Reading MaxWear...')
output = pd.read_csv('train1/00_Wear_data.csv').loc[:, 'MaxWear']
...
print('sg_A|\t\t1:', sg_A[0].size, '\t26:', sg_A[25].size, '\t27:', sg_A[26].size, '\t46:', sg_A[45].size)
print('--------'*10)
print('sg_B|\t\t1:', sg_B[0].size, '\t26:', sg_B[25].size, '\t27:', sg_B[26].size, '\t46:', sg_B[45].size)
print('sg_C|\t\t1:', sg_C[0].size, '\t26:', sg_C[25].size, '\t27:', sg_C[26].size, '\t46:', sg_C[45].size)
print('sg_D|\t\t1:', sg_D[0].size, '\t26:', sg_D[25].size, '\t27:', sg_D[26].size, '\t46:', sg_D[45].size)
print('sg_E|\t\t1:', sg_E[0].size, '\t26:', sg_E[25].size, '\t27:', sg_E[26].size, '\t46:', sg_E[45].size)
print('sg_F|\t\t1:', sg_F[0].size, '\t26:', sg_F[25].size, '\t27:', sg_F[26].size, '\t46:', sg_F[45].size)
print('sg_G|\t\t1:', sg_G[0].size, '\t26:', sg_G[25].size, '\t27:', sg_G[26].size, '\t46:', sg_G[45].size)
print('sg_H|\t\t1:', sg_H[0].size, '\t26:', sg_H[25].size, '\t27:', sg_H[26].size, '\t46:', sg_H[45].size)
print('sg_I|\t\t1:', sg_I[0].size, '\t26:', sg_I[25].size, '\t27:', sg_I[26].size, '\t46:', sg_I[45].size)
...
for i in range(len(sg_B)):
    if sg_B[i].size < max_num:
        sg_B[i] = np.append(sg_B[i], [[sg_B[i][-1]] * (max_num - sg_B[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(sg_C)):
    if sg_C[i].size < max_num:
        sg_C[i] = np.append(sg_C[i], [[sg_C[i][-1]] * (max_num - sg_C[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(sg_D)):
    if sg_D[i].size < max_num:
        sg_D[i] = np.append(sg_D[i], [[sg_D[i][-1]] * (max_num - sg_D[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(sg_E)):
    if sg_E[i].size < max_num:
        sg_E[i] = np.append(sg_E[i], [[sg_E[i][-1]] * (max_num - sg_E[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(sg_F)):
    if sg_F[i].size < max_num:
        sg_F[i] = np.append(sg_F[i], [[sg_F[i][-1]] * (max_num - sg_F[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(sg_G)):
    if sg_G[i].size < max_num:
        sg_G[i] = np.append(sg_G[i], [[sg_G[i][-1]] * (max_num - sg_G[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(sg_H)):
    if sg_H[i].size < max_num:
        sg_H[i] = np.append(sg_H[i], [[sg_H[i][-1]] * (max_num - sg_H[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(sg_I)):
    if sg_I[i].size < max_num:
        sg_I[i] = np.append(sg_I[i], [[sg_I[i][-1]] * (max_num - sg_I[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(spike_A)):
    if spike_A[i].size < max_num:
        spike_A[i] = np.append(spike_A[i], [[spike_A[i][-1]] * (max_num - spike_A[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(spike_B)):
    if spike_B[i].size < max_num:
        spike_B[i] = np.append(spike_B[i], [[spike_B[i][-1]] * (max_num - spike_B[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(spike_C)):
    if spike_C[i].size < max_num:
        spike_C[i] = np.append(spike_C[i], [[spike_C[i][-1]] * (max_num - spike_C[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(spike_D)):
    if spike_D[i].size < max_num:
        spike_D[i] = np.append(spike_D[i], [[spike_D[i][-1]] * (max_num - spike_D[i].size)])
print('cost time: ', time.time() - s)
...
for i in range(len(CISP_A)):CISP_A[i] = np.insert(CISP_A[i], 0, 0)
for i in range(len(CISP_B)):CISP_B[i] = np.insert(CISP_B[i], 0, 0)
for i in range(len(CISP_C)):CISP_C[i] = np.insert(CISP_C[i], 0, 0)
for i in range(len(CISP_D)):CISP_D[i] = np.insert(CISP_D[i], 0, 0)
...
""""""
print('A...', end='')
final_abs_A = [[abs(CISP_A[j][i*3+1:i*3+4]).sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_A))]
print('cost time: ', time.time() - s)

s = time.time()
print('B...', end='')
final_abs_B = [[abs(CISP_B[j][i*3+1:i*3+4]).sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_B))]
print('cost time: ', time.time() - s)

s = time.time()
print('C...', end='')
final_abs_C = [[abs(CISP_C[j][i*3+1:i*3+4]).sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_C))]
print('cost time: ', time.time() - s)

s = time.time()
print('D...', end='')
final_abs_D = [[abs(CISP_D[j][i*3+1:i*3+4]).sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_D))]
print('cost time: ', time.time() - s)
...
s = time.time()
print('A...', end='')
final_A = [[CISP_A[j][i*3+1:i*3+4].sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_A))]
print('cost time: ', time.time() - s)

s = time.time()
print('B...', end='')
final_B = [[CISP_B[j][i*3+1:i*3+4].sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_B))]
print('cost time: ', time.time() - s)

s = time.time()
print('C...', end='')
final_C = [[CISP_C[j][i*3+1:i*3+4].sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_C))]
print('cost time: ', time.time() - s)

s = time.time()
print('D...', end='')
final_D = [[CISP_D[j][i*3+1:i*3+4].sum() for i in range(0, CISG_B[j].size-1)] for j in range(len(CISP_D))]
print('cost time: ', time.time() - s)
...
for i in final_abs_A: i = np.array(i.insert(0, 0))
for i in final_abs_B: i = np.array(i.insert(0, 0))
for i in final_abs_C: i = np.array(i.insert(0, 0))
for i in final_abs_D: i = np.array(i.insert(0, 0))
for i in final_A: i = np.array(i.insert(0, 0))
for i in final_B: i = np.array(i.insert(0, 0))
for i in final_C: i = np.array(i.insert(0, 0))
for i in final_D: i = np.array(i.insert(0, 0))
...
np.save('spike_A', spike_A)
np.save('spike_B', spike_B)
np.save('spike_C', spike_C)
np.save('spike_D', spike_D)
np.save('spike_abs_A', spike_abs_A)
np.save('spike_abs_B', spike_abs_B)
np.save('spike_abs_C', spike_abs_C)
np.save('spike_abs_D', spike_abs_D)

np.save('spike_abs_B_lower_noise', spike_abs_B_lower_noise)
np.save('spike_B_lower_noise', spike_B_lower_noise)
np.save('spike_abs_C_lower_noise', spike_abs_C_lower_noise)
np.save('spike_C_lower_noise', spike_C_lower_noise)

np.save('BCD_distance', BCD_distance)
np.save('BCD_abs_distance', BCD_abs_distance)
np.save('BDF_distance', BDF_distance)
