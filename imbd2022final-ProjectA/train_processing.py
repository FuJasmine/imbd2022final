...
#
anomaly_train2['anomaly_accumulation_hour'] = anomaly_train2['anomaly_accumulation_hour']+1

cooler = pd.read_csv("train1/cooler.csv")
power = pd.read_csv("train1/power.csv")


accumulation_hour1 = accumulation_hour1.rename(columns={'accumulation_hour': 'anomaly_accumulation_hour'})
accumulation_hour2 = accumulation_hour2.rename(columns={'accumulation_hour': 'anomaly_accumulation_hour'})

#
accumulation_hour2['anomaly_accumulation_hour'] = accumulation_hour2['anomaly_accumulation_hour']+2
...
anomaly_train2.loc[:, 'lamp_id'] = pd.Series([list(map(int, string.split('_'))) for string in anomaly_train2.loc[:, 'lamp_id']])

accumulation_hour1['lamp_id'] = pd.Series([[] for i in range(accumulation_hour1.shape[0])])
accumulation_hour2['lamp_id'] = pd.Series([[] for i in range(accumulation_hour2.shape[0])])
...
anomaly_train1['    check'] = pd.Series(['N' if anomaly_train1.loc[i, 'anomaly_accumulation_hour'] > anomaly_train1.loc[i, 'oven hour'] else ' ' for i in range(anomaly_train1.shape[0])])
anomaly_train2['    check'] = pd.Series(['N' if anomaly_train2.loc[i, 'anomaly_accumulation_hour'] > anomaly_train2.loc[i, 'oven hour'] else ' ' for i in range(anomaly_train2.shape[0])])
...
anomaly_train1['total count'] = anomaly_train1['A count'] + anomaly_train1['B count']
anomaly_train1 = anomaly_train1.drop(columns=['lamp_id', 'anomaly_total_number', 'A count', 'B count'])
...
anomaly_train2['total count'] = anomaly_train2['A count'] + anomaly_train2['B count']
anomaly_train2 = anomaly_train2.drop(columns=['lamp_id', 'anomaly_total_number', 'A count', 'B count'])
...
anomaly_train = pd.concat([anomaly_train1, anomaly_train2], ignore_index=True).sort_values(by=['oven_id', 'layer_id', 'anomaly_accumulation_hour'])
