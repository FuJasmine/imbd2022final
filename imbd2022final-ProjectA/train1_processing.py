...
accumulation_hour = accumulation_hour.rename(
...
anomaly_train.loc[:, 'lamp_id'] = pd.Series(
        [list(map(int, string.split('_'))) for string in anomaly_train.loc[:, 'lamp_id']])

accumulation_hour['lamp_id'] = pd.Series(
        [[] for i in range(accumulation_hour.shape[0])])
...
anomaly_train['    check'] = pd.Series(['N' if anomaly_train.loc[i, 'anomaly_accumulation_hour']
                                       > anomaly_train.loc[i, '5/4 oven hour'] else ' ' for i in range(anomaly_train.shape[0])])
...
anomaly_train['total count'] = anomaly_train['A count'] + \
        anomaly_train['B count']
anomaly_train = anomaly_train.drop(
