...
for hour in df.loc[:, 'anomaly_accumulation_hour']:
    ...
    for i in power.loc[:, 'accumulation_hour']:
        if index == 0:
            if hour >= i[1]:
                powers[0] += (i[1] - i[0]) * np.power(power.loc[index,
                                                                'power_setup(other_lamp)'], 10)
                powers[1] += (i[1] - i[0]) * np.power(power.loc[index,
                                                                'power_setup(lamp_1_2_60_61_62_63_121_122)'], 10)
            else:
                powers[0] = hour * \
                    np.power(power.loc[index, 'power_setup(other_lamp)'], 10)
...
cooling_system['upper_water_volume'] = pd.Series(
    [i for oven in cooler.columns[1:] for i in cooler.loc[0:18, oven]])
cooling_system['lower_water_volume'] = pd.Series(
    [i for oven in cooler.columns[1:] for i in cooler.loc[1:19, oven]])
...
df = df.drop(columns=['date', 'oven_id', 'layer_id'])
